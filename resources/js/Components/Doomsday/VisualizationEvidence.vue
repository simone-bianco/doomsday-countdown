...
<template>
    <div v-if="reasoningText || validSources.length" class="space-y-3 border-t border-white/10 pt-3 text-xs leading-relaxed text-white/55">
        <p v-if="reasoningText">
            <span class="font-semibold text-white/75">Reasoning:</span>
            {{ reasoningText }}
        </p>
        <div v-if="validSources.length">
