...
<template>
    <section class="space-y-4">
        ...
        >
            ...
            <template #cell-meta="{ item }"><div class="flex flex-wrap gap-2"><Badge :label="String(item.locale)" variant="soft" /><Badge :label="String(item.content_type ?? 'article')" variant="soft" /><Badge v-if="item.external_provider" :label="String(item.external_provider)" variant="soft" /><Badge :label="item.is_featured ? 'featured' : 'standard'" :color="item.is_featured ? 'success' : 'secondary'" variant="soft" /></div><div class="mt-1 text-sm text-ui-muted-foreground">{{ displayPublishedAt(String(item.published_at ?? '') || null) }}</div></template>
