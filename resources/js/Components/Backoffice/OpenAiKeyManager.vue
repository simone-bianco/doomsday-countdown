...
import { Badge, Button, DataTable, Modal, SearchBox, TextInput, Toggle } from '@simone-bianco/vue-ui-components';
...
const rows = computed<Record<string, unknown>[]>(() => props.apiKeys
    .filter((apiKey) => matchesSearch(apiKey, normalizedSearchQuery.value))
    .map((apiKey) => ({ ...apiKey })));
const columns = [
...
function matchesSearch(apiKey: ApiKey, query: string): boolean {
    if (query === '') {
        return true;
    }
...
<template>
    <div class="space-y-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <SearchBox v-model="searchQuery" placeholder="Search OpenAI keys" :ui="backofficeTableSearchUi" />
            <Button :icon="Plus" @click="openCreate">Register key</Button>
        </div>

        <DataTable :items="rows" :columns="columns" item-key="id" density="comfortable" enable-row-click :ui="backofficeInteractiveTableUi" @row-click="(item) => edit(asApiKey(item))">
            ...
            <template #empty>
                <p class="text-sm text-ui-muted-foreground">No OpenAI keys match this search.</p>
            </template>
        </DataTable>
    </div>
